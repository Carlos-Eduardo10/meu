...
$mesesDoAnoExibirGraph = json_encode(array_values($mesesDoAno));

$consumoMensal = Dashboard::consumoPorMes($year);
$consumoMensalExibirGraph = json_encode($consumoMensal['consume']);

$mediaMensalExibirGraph = json_encode($media['monthlyValues']);

$somaMediaMensal = (array_sum($meta['monthlyValues']) / 12); 
die();
?>
<script>
    var consumo = <?= $consumoMensalExibirGraph ?>;
    var meta = <?= ($somaMediaMensal > 1 ) ? $mediaMensalExibirGraph : ""?>;
    var options = {
        series: [{
                name: 'Consumo',
                type: 'bar',
        ...
        colors: [
            function({
                ...
                if (seriesIndex === 0) {
                    ...
                    if (metaValue > 1 && value > metaValue) {
        ...
        xaxis: {
            categories: <?= $mesesDoAnoExibirGraph ?>,
        },
        yaxis: {
            title: {
                text: 'm3'
